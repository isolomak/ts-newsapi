...
export default class NewsAPI {
	...
	public async getTopHeadlines(params: INewsApiTopHeadlinesParams = { }): Promise<INewsApiResponse> {
		...
		const queries: Array<string> = [ querystring.stringify({ apiKey: this._apiKey }) ];

		if (params.q) {
			queries.push(
				querystring.stringify( { q: params.q } )
			);
		}

		if (params.country) {
			queries.push(
				querystring.stringify( { country: params.country } )
			);
		}

		if (params.category) {
			queries.push(
				querystring.stringify( { category: params.category } )
			);
		}
		...
		if (params.pageSize) {
			queries.push(
				querystring.stringify({ pageSize: params.pageSize })
			);
		}

		if (params.page) {
			queries.push(
				querystring.stringify({ page: params.page })
			);
		}
	...
	public async getEverything(params: INewsApiEverythingParams = { }): Promise<INewsApiResponse> {
		...
		if (params.qInTitle) {
			queries.push(
				querystring.stringify({ qInTitle: params.qInTitle })
			);
		}
		...
		if (params.excludeDomains && params.excludeDomains.length) {
			queries.push(
				querystring.stringify({ excludeDomains: params.excludeDomains.join(',') })
			);
		}

		if (params.from) {
			queries.push(
				querystring.stringify({ from: params.from })
			);
		}

		if (params.to) {
			queries.push(
				querystring.stringify({ to: params.to })
			);
		}

		if (params.language) {
			queries.push(
				querystring.stringify({ language: params.language })
			);
		}

		if (params.sortBy) {
			queries.push(
				querystring.stringify({ sortBy: params.sortBy })
			);
		}
